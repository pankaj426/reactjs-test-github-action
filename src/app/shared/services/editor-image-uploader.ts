...
class EditorImageUpload {
  ...
  _initRequest() {
    ...
    xhr.open(
      "POST",
      this.apiOrganisationBaseUrl + url,
      true
    );
  ...
  _sendRequest(file) {
    ...
    data.append("file", file, doc_name);
    data.append("title", doc_name);
    data.append("category", "attachments");
...
export function CustomImageUploadPlugin(editor) {
  editor.plugins.get('FileRepository').createUploadAdapter = (loader) => {
    // Configure the URL to the upload script in your back-end here!
    return new EditorImageUpload(loader);
  };
}
